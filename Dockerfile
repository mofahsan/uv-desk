...
# Set APP_ENV to prod to prevent loading dev dependencies
ENV APP_ENV=prod
RUN cd /var/www/uvdesk/ && composer install --no-dev --optimize-autoloader

# Ensure correct ownership for uvdesk user and create cache directory
RUN mkdir -p /var/www/uvdesk/public/cache/images && \
