...
setup_mysql() {
    ...
    # Try to connect with root password first, then without
    ...
    if ! $MYSQL_CMD -e "SELECT 1;" >/dev/null 2>&1; then
        ...
        if ! $MYSQL_CMD -e "SELECT 1;" >/dev/null 2>&1; then
            echo -e "${COLOR_YELLOW}Cannot connect to MySQL. Trying to reset root password...${COLOR_NC}"
            # Stop MySQL and restart in safe mode to reset password
            service mysql stop
    ...
    if $MYSQL_CMD -e "CREATE DATABASE IF NOT EXISTS $MYSQL_DATABASE;"; then
    ...
    if $MYSQL_CMD -e "SELECT User FROM mysql.user WHERE User='$MYSQL_USER';" | grep -q "$MYSQL_USER"; then
        echo -e "${COLOR_YELLOW}User '$MYSQL_USER' already exists${COLOR_NC}"
    else
        echo -e "${COLOR_BLUE}Creating MySQL user '$MYSQL_USER'...${COLOR_NC}"
        # MySQL 8.0 syntax
        if $MYSQL_CMD -e "CREATE USER '$MYSQL_USER'@'localhost' IDENTIFIED BY '$MYSQL_PASSWORD';"; then
    ...
    if $MYSQL_CMD -e "GRANT ALL PRIVILEGES ON $MYSQL_DATABASE.* TO '$MYSQL_USER'@'localhost';"; then
    ...
    echo -e "${COLOR_BLUE}Ensuring root password is set...${COLOR_NC}"
    if $MYSQL_CMD -e "ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY '$MYSQL_ROOT_PASSWORD';"; then
        echo -e "${COLOR_GREEN}Root password verified/updated${COLOR_NC}"
    else
        echo -e "${COLOR_YELLOW}Root password already set correctly${COLOR_NC}"
    fi
...
# Create and set permissions for image cache directory
if [ ! -d "/var/www/uvdesk/public/cache/images" ]; then
    mkdir -p /var/www/uvdesk/public/cache/images
    echo -e "${COLOR_GREEN}Created image cache directory${COLOR_NC}"
fi
# Always ensure cache directory has correct permissions
# Set ownership to uvdesk user (Apache is running as uvdesk)
chmod -R 775 /var/www/uvdesk/public/cache
chown -R uvdesk:uvdesk /var/www/uvdesk/public/cache
echo -e "${COLOR_GREEN}Set permissions for image cache directory (uvdesk:uvdesk)${COLOR_NC}"
